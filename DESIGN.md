...
6. **High-Density Protocol Tables & Dose Badges**: `.protocol-table-wrapper` with responsive horizontal scrolling, `.protocol-table` headers and cells, and monospace `.dose-badge` tags across Seizure, Drip, Tox, PSA, Vitals, DKA, Asthma, and Broselow Equipment drawers.
...
11. **Category Jump Bar & Module Quick-Filter**: Dedicated `.category-bar` with tactile `.category-pill` buttons (🌟 *All Modules*, ⚡ *Emergency Resus*, 💊 *Meds & Fluids*, 🩺 *Protocols & Vitals*) providing instant semantic navigation with dimmed state filtering for reduced visual clutter (Key Decision #47).
12. **Sticky First-Column Protocol Tables & Subtle Alternating Rows**: Responsive `.protocol-table` data grids featuring `position: sticky; left: 0` headers and first columns with `color-mix` zebra striping and hover highlights, ensuring drug names remain visible during horizontal scrolling on narrow viewports (Key Decision #47).
13. **Combobox Drug Category Filter Pills & Search Match Highlighting**: Quick-filter chips (`.drug-filter-pill`) above Dose and ATB search boxes, paired with sanitized `<mark class="search-match">` query highlighting for rapid bedside drug verification (Key Decision #47).
14. **Tactile Interaction & Accessible Focus System**: Comprehensive `:focus-visible` offset rings (3px outline, 2px offset) and physical compression tap states (`:active { transform: scale(0.97); }`) across all interactive buttons, pills, and switches (Key Decision #47).
15. **Asthma Protocol Stage Cards & HFNC Settings Panel**: `.stage-card` containers rendering 6-stage stepwise protocol with embedded `.protocol-table` drug dosing grids. Dedicated HFNC card with sky-blue left border (`#0ea5e9`) and auto-calculated settings (Flow, FiO₂, Temperature, NEB flow). Color-coded severity assessment table headers (green Mild–Moderate `#dcfce7`, yellow Severe `#fef9c3`, red Life-Threatening `#fee2e2`). Disposition criteria rendered as responsive 3-column grid cards with matching severity color coding (Key Decision #48).

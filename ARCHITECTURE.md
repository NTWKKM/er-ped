...
31. **Design Mockup v2 Integration & 3-State Theme Engine**: Merges design tokens from `design-mockup-v2.html` into `index.html`. Expands theme engine to 3-state cycle (`Light` → `Dark` → `Mono`) with persistent state in `localStorage` (`er_ped_theme`). Implements 2-row topbar with decluttered `Reference ▾` popover and `⋯` Overflow menu (`Print` / `Language`). Refactors Broselow chip badges to theme-aware CSS custom properties (`var(--panel)`, `var(--ink)`), ensuring contrast and legibility across all themes. Groups navigation tabs into 2 `.tabs` containers (Primary & Specialty) with `title` tooltips for `[Alt+N]` shortcuts and VS15 text presentation locking (`\FE0E`) on all status glyphs (`✓\FE0E`, `▲\FE0E`, `✕\FE0E`, `ℹ\FE0E`).
